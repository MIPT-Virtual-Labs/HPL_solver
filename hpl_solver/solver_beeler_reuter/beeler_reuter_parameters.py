...
class InputParameters(BaseModel):
    ...
    def check_Istim(cls, params):
        ...
        if IstimStart >= IstimEnd:
            raise ValueError(f"Parameter 'IstimStart'={IstimStart} should be less than 'IstimEnd'={IstimEnd}")
        return params
...
class SolverParameters(BaseModel):
    ...
    def check_method(cls, method):
        ...
        if method not in method_list:
            raise ValueError(f"Undefined method : {method}, must be one of the list {method_list}")
        return method
